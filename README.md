...
的进程没有正确处理信号量，会造成无法停止容器。而应用开发者一般不会，也不需要关心
这两件事。因此，我们需要为镜像设置一个默认的 PID 为 1 的进程来管理子进程和信号量。
这样，当应用开发者不关心自己的进程 PID 为几时，由此 PID 为 1 的进程来管理
开发者的进程、传递信号量，并在应用不响应信号量时做适当的操作；当应用开发者
...
子进程退出后收割子进程，并正确的处理信号量；`s6-log` 可以收集 stdout/stderr 并自动 rotate，
